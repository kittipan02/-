...
<body>
    ...
    <form method="post" action="repair_sale.php">
    ...
    WHERE (rs.rs_name = 'รอจำหน่าย') AND (rr.equipment_number LIKE '%$search%' OR rr.item LIKE '%$search%' OR dp.dp_name LIKE '%$search%' OR u.u_fname LIKE '%$search%' OR u.u_lname LIKE '%$search%' OR et.et_name LIKE '%$search%' OR b.brand_name LIKE '%$search%' OR rr.repair_details LIKE '%$search%')
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td><center>";
            echo htmlspecialchars($row["technician_name"]);

            echo "<td><center>";

            if ($row["status_name"] != "จำหน่ายเสร้จสิ้น") {
                // Display the form to update status
                echo "<form method='post' action='update_sale.php'>";
